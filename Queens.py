...
def queens_max(state):
    #Customized fitness function for maximizing the queens problem 
    #Reference: mlrose Documentation Release 1.3.0
    fitness = 0
...
fitness_Queens = ml.CustomFitness(queens_max)



def solve_opt(input_size):
    
    problem_Queens = ml.DiscreteOpt(input_size,fitness_Queens)

    '''
    Randomize hill climbing
    '''
    start = time.time()
    best_state_rhc, best_fitness_rhc,fitness_curve_rhc = ml.random_hill_climb(problem_Queens, 
    ...
    best_state_sa, best_fitness_sa,fitness_curve_sa = ml.simulated_annealing(problem_Queens, schedule = schedule,
    max_attempts = 10, max_iters =10000,curve=True,random_state=seed)
    end = time.time()
    ...
    """
    start = time.time()
    best_state_ga, best_fitness_ga,fitness_curve_ga = ml.genetic_alg(problem_Queens,  pop_size=200, mutation_prob=0.1,
    ...
    """
    start = time.time()
    best_state_mimic, best_fitness_mimic,fitness_curve_mimic = ml.mimic(problem_Queens,  pop_size=200, keep_pct=0.2,
    max_attempts = 10, max_iters =1000,curve=True,random_state=seed)
    end = time.time()
    ...
    plot_fitness_curve(data,'Queens problem with '+str(input_size)+' inputs')
...
#MIMIC is clearly the best (by the combination of cpu time, iterations and fitness)
